		...
		<view class="content">
			...
			<view class="bar">
				<view class="iconSpan" v-for="(item, index) in contentBar" :key="index">
					<image :src="item.src" class="iconBarImg"></image>
					<view>{{item.name}}</view>
				</view>
			</view>
			...
			<view class="used">
				<view class="usedImg" @tap="goUrl('../info/index')">
					...
					<image class="icon" :src="`${imgSrc}icon/sanjiao.png`"></image>
	...
	export default {
		...
		methods: {
			...
			recChange (val) {
				// 将数组前两条数据放到末尾
				if (val === 'recom') {
					let array = this.recommend.list.splice(0,2)
					this.recommend.list = [...this.recommend.list,...array]
				}
			}
		},
		components: {
